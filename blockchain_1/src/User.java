...
public class User {
    ...
    public void verifyBlockChain() throws Exception {
        for (int i = 0; i < this.blockchain.chain.size()-1; i++) {
            ...
            if (b.hash == null) {
                boolean flag = this.verifyBlock(this.blockchain.chain.get(i));
                String previous = b.previous;
                if (!flag) {
                    this.blockchain.chain.remove(i);
                    this.blockchain.chain.get(i).previous = previous;
                    i--;
                }
            }
        }
        boolean flag = this.verifyBlock(this.getLastBlock());
        if(!flag){
